...
# Verificar si las predicciones pertenecen a las clases conocidas
clases_conocidas = set(label_encoder.classes_)
predicciones_validas = [etiqueta for etiqueta in etiquetas_predichas if etiqueta in clases_conocidas]

# Mostrar las predicciones válidas
print("Predicciones válidas del modelo en el nuevo conjunto de datos:")
for i, etiqueta in enumerate(predicciones_validas):
    print(f"Muestra {i+1}: {etiqueta}")

# Calcular el porcentaje de predicciones válidas
porcentaje_validas = (len(predicciones_validas) / len(etiquetas_predichas)) * 100

print(f"Porcentaje de predicciones válidas: {porcentaje_validas:.2f}%")
